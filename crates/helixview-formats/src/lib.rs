...
pub fn save(aln: &Alignment, path: &Path) -> Result<()> {
    ...
    match ext.as_str() {
        "fasta" | "fa" | "fna" | "faa" | "ffn" | "frn" =>
            fasta::write_file(aln, path),
        "gb" | "gbk" | "genbank" =>
            genbank::write_file(aln, path),
        "aln" =>
            clustal::write_file(aln, path),
        "pir" | "nbrf" =>
            nbrf::write_file(aln, path),
        "phy" | "phylip" =>
            phylip::write_file(aln, path),
        "nex" | "nexus" | "nxs" =>
            nexus::write_file(aln, path),
        "msf" =>
            msf::write_file(aln, path),
        "embl" | "dat" =>
            embl::write_file(aln, path),
        _ =>
            fasta::write_file(aln, path),
    }
}
