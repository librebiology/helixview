...
pub fn parse_features(raw: &str) -> Vec<Feature> {
    ...
    for line in raw.lines() {
        if line.trim_start().starts_with("Location") { continue; }

        let leading = line.bytes().take_while(|&b| b == b' ').count();
        if leading >= line.len() { continue; }
        ...
        if leading == 5 && !content.starts_with('/') {
            ...
            if in_location {
                ...
                if !cur_qval.is_empty() { cur_qval.push(' '); }
                cur_qval.push_str(c);
            }
        }
    }
    ...
    const CYCLE: [(u8, u8, u8); 10] = [
        ...
        (215,  50, 120), (189, 166,  12), (100, 160, 100), (180,  80,  40),
    ];
    let mut ci = 0usize;
...
fn flush_qualifier(qname: &mut Option<String>, qval: &mut String, quals: &mut Vec<(String, String)>) {
...
fn build_feature(key: &str, location: &str, quals: &[(String, String)]) -> Option<Feature> {
    ...
    if matches!(feature_type, FeatureType::Source) { return None; }
...
fn parse_location(loc: &str) -> Option<(usize, usize, FeatureDirection)> {
    ...
    if loc.starts_with("join(") {
        ...
        if min_start == usize::MAX { return None; }
...
Location/Qualifiers
    ...
    fn location_1based_to_0based() {
        ...
        let cds = feats.iter().find(|f| matches!(f.feature_type, FeatureType::CDS)).unwrap();
...
Location/Qualifiers
    ...
    fn feature_type_mapped_correctly() {
        assert!(parse_features(SAMPLE).iter().any(|f| matches!(f.feature_type, FeatureType::CDS)));
    }
}
