...
use iced::{
    ...
    widget::{
        ...
        column, container, row, scrollable, text, text_input, horizontal_space,
    },
...
const RE_PALETTE: [Color; 20] = [
    ...
    Color { r: 0.10, g: 0.60, b: 0.10, a: 1.0 },
    Color { r: 0.10, g: 0.30, b: 0.85, a: 1.0 },
    Color { r: 0.90, g: 0.50, b: 0.05, a: 1.0 },
    Color { r: 0.55, g: 0.10, b: 0.70, a: 1.0 },
    Color { r: 0.05, g: 0.65, b: 0.65, a: 1.0 },
    Color { r: 0.65, g: 0.35, b: 0.05, a: 1.0 },
    Color { r: 0.65, g: 0.05, b: 0.40, a: 1.0 },
    Color { r: 0.20, g: 0.55, b: 0.25, a: 1.0 },
    Color { r: 0.75, g: 0.65, b: 0.05, a: 1.0 },
    Color { r: 0.15, g: 0.50, b: 0.75, a: 1.0 },
    Color { r: 0.85, g: 0.30, b: 0.60, a: 1.0 },
    Color { r: 0.35, g: 0.65, b: 0.35, a: 1.0 },
    Color { r: 0.60, g: 0.25, b: 0.75, a: 1.0 },
    Color { r: 0.25, g: 0.70, b: 0.55, a: 1.0 },
    Color { r: 0.75, g: 0.40, b: 0.20, a: 1.0 },
    Color { r: 0.45, g: 0.20, b: 0.60, a: 1.0 },
    Color { r: 0.20, g: 0.55, b: 0.50, a: 1.0 },
    Color { r: 0.70, g: 0.15, b: 0.15, a: 1.0 },
    Color { r: 0.30, g: 0.30, b: 0.70, a: 1.0 },
];
...
pub fn plasmid_view<'a>(
    ...
    let label = if seq_name.len() > 22 { format!("{}…", &seq_name[..21]) } else { seq_name.to_string() };
    ...
    let feat_label = if show_features { "Feat ●" } else { "Feat ○" };
    ...
    let toolbar = container(
        row![
            ...
            text(format!("{:.0}%", zoom * 100.0)).size(11).color(crate::theme::palette::TEXT_DIM),
    ...
    let re_sites: Vec<(String, usize, Color)> = if show_re {
        match &re_sites_cache {
            Some(cached) => cached.iter()
                .map(|s| {
                    let color = RE_PALETTE[ENZYMES[s.enzyme_idx].color_idx % RE_PALETTE.len()];
                    (ENZYMES[s.enzyme_idx].name.to_string(), s.true_pos + 1, color)
                })
                .collect(),
    ...
    let feat_panel = feature_panel(aln, seq_idx, show_re, selected_feat, edit_name, edit_start, edit_end, edit_color, edit_hex, re_sites, re_computing);
...
fn feature_panel<'a>(
    ...
    let features = aln.sequences.get(seq_idx)
    ...
    for (i, feat) in features.iter().enumerate() {
        if matches!(feat.feature_type, FeatureType::Source) { continue; }
        ...
        let swatch: Element<Message> = container(iced::widget::Space::with_width(10))
            ...
            .style(move |_| container::Style {
                background: Some(Background::Color(Color { r: fc.r, g: fc.g, b: fc.b, a: 1.0 })),
                border: Border { color: Color::from_rgb(0.5, 0.5, 0.5), width: 0.5, radius: 2.0.into() },
                ..Default::default()
            })
            .into();
            ...
            button(
                container(
                    row![
                        ...
                        horizontal_space(),
                        text(pos_label).size(10).color(if is_sel { Color::from_rgba(1.0,1.0,1.0,0.75) } else { palette::TEXT_DIM }),
                        del_btn,
                    ]
                    .spacing(4)
                    .align_y(iced::Alignment::Center)
    ...
    let edit_panel: Element<Message> = if selected_feat.is_some() {
        container(
            column![
                container(
                    text("Edit feature").size(12).color(palette::TEXT)
                )
                .padding([4, 8])
                .style(|_| container::Style {
                    background: Some(Background::Color(palette::HEADER_BG)),
                ...
                container(
                    column![
                        row![
                            text("Name").size(11).color(palette::TEXT_DIM).width(Length::Fixed(45.0)),
                        ...
                        row![
                            text("Start").size(11).color(palette::TEXT_DIM).width(Length::Fixed(45.0)),
                        ...
                        row![
                            text("End").size(11).color(palette::TEXT_DIM).width(Length::Fixed(45.0)),
...

                        {
                            ...
                            let make_swatch = |c: CoreColor| -> Element<'static, Message> {
                                let ic = Color { r: c.r, g: c.g, b: c.b, a: 1.0 };
                                ...
                                button(iced::widget::Space::with_width(Length::Fill))
                                    ...
                                    .style(move |_, _| button::Style {
                                        ...
                                        border: Border {
                                            color: if selected { Color::WHITE } else { Color::from_rgba(0.0, 0.0, 0.0, 0.35) },
                                            width: if selected { 2.5 } else { 1.0 },
                                            radius: 3.0.into(),
                                        },
                                        shadow: if selected {
                                            iced::Shadow { color: Color::from_rgba(0.0, 0.0, 0.0, 0.4), offset: iced::Vector::new(0.0, 0.0), blur_radius: 3.0 }
                                        } else {
                                            iced::Shadow::default()
                                        },
                                        ..Default::default()
                            ...
                            let row1: Vec<Element<Message>> = FEAT_PALETTE[..8].iter().copied().map(&make_swatch).collect();
                            let row2: Vec<Element<Message>> = FEAT_PALETTE[8..].iter().copied().map(&make_swatch).collect();
                            row![
                                text("Color").size(11).color(palette::TEXT_DIM).width(Length::Fixed(45.0)),
                                column![
...

                        {
                            ...
                            let preview_ic = Color { r: edit_color.r, g: edit_color.g, b: edit_color.b, a: 1.0 };
                            row![
                                text("Hex").size(11).color(palette::TEXT_DIM).width(Length::Fixed(45.0)),
                                container(iced::widget::Space::with_width(Length::Fill))
                                    .width(18)
                                    .height(18)
                                    .style(move |_| container::Style {
                                        background: Some(Background::Color(preview_ic)),
                                        border: Border { color: Color::from_rgba(0.0,0.0,0.0,0.3), width: 1.0, radius: 3.0.into() },
                                        ..Default::default()
                                    }),
    ...
    let re_section: Element<Message> = if re_computing {
        container(
            text("⏳ Computing RE sites…").size(11).color(palette::TEXT_DIM),
        )
        .padding([6, 8])
        .width(Length::Fill)
        .into()
        ...
        for (i, (name, pos, color)) in re_sites.into_iter().enumerate() {
            let bg = if i % 2 == 0 { Color::WHITE } else { Color::from_rgb(0.97, 0.97, 0.99) };
            let dot: Element<Message> = container(iced::widget::Space::with_width(8))
                .width(8).height(8)
                .style(move |_| container::Style {
                    background: Some(Background::Color(color)),
                    border: Border { color: Color::from_rgba(0.0,0.0,0.0,0.2), width: 0.5, radius: 2.0.into() },
                    ..Default::default()
                })
                .into();
...
impl canvas::Program<Message> for PlasmidCanvas {
    ...
    fn draw(
        ...
        let geom = state.cache.draw(renderer, snapped, |frame| {
            draw_plasmid(frame, snapped, visible, &aln, seq_idx,
                         show_features, show_re, selected_feat, zoom, pan_x, pan_y,
...
impl PlasmidCanvas {
    ...
    fn hit_test(&self, size: Size, pos: Point) -> Option<Message> {
        if !self.show_features { return None; }
        let seq = self.aln.sequences.get(self.seq_idx)?;

        let raw_len: usize = seq.residues.iter()
        ...
        if (dist - feat_r).abs() > feat_half + 8.0 { return None; }
...
fn draw_plasmid(
    ...
    let (major_step, minor_step) = tick_steps(seq_len);
    {
        ...
        while pos <= seq_len {
            ...
            let (r_inner, r_outer) = if is_major { (r - 10.0, r + 10.0) } else { (r - 5.0, r + 5.0) };
            let p0 = pt(cx, cy, r_inner, a);
            let p1 = pt(cx, cy, r_outer, a);
            let tick = Path::new(|b| { b.move_to(p0); b.line_to(p1); });
            let color = if is_major {
                Color::from_rgb(0.35, 0.35, 0.40)
            } else {
                Color::from_rgb(0.70, 0.70, 0.74)
            };
            frame.stroke(&tick, Stroke::default().with_color(color).with_width(if is_major { 1.5 } else { 0.8 }));
            ...
            pos += minor_step;
            if pos > seq_len && pos - minor_step < seq_len { pos = seq_len; }
            else if pos > seq_len { break; }
        }
    }
    ...
    if show_features {
        ...
        for (feat_idx, feat) in seq.features.iter().enumerate() {
            if matches!(feat.feature_type, FeatureType::Source) { continue; }
            if feat.start > feat.end && feat.end == 0 { continue; }
            ...
            if is_sel {
                ...
                frame.stroke(&halo, Stroke::default()
            ...
            if !matches!(feat.direction, FeatureDirection::None) {
                ...
                let (tx, ty) = if fwd { (-tip_a.sin(), tip_a.cos()) } else { (tip_a.sin(), -tip_a.cos()) };
                ...
                let arrow  = Path::new(|b| { b.move_to(tip_pt); b.line_to(b1); b.line_to(b2); b.close(); });
                frame.fill(&arrow, fc);
            }
            ...
            if !feat.name.is_empty() {
                ...
                let chip_x  = if on_right { lp.x - 3.0 } else { lp.x - chip_w + 3.0 };
                let chip_y  = lp.y - chip_h * 0.5;
                let chip_path = Path::new(|b| {
                    let r2 = (chip_h * 0.3).min(4.0);
                    b.rounded_rectangle(Point::new(chip_x, chip_y), Size::new(chip_w, chip_h), iced::border::Radius::new(r2));
                });
                frame.fill(&chip_path, Color::from_rgba(1.0, 1.0, 1.0, 0.88));
                frame.stroke(&chip_path, Stroke::default()
...
fn tick_steps(len: usize) -> (usize, usize) {
    if      len <       1_000 { (100,      20)      }
    else if len <       5_000 { (500,      100)     }
    else if len <      20_000 { (1_000,    200)     }
    else if len <     100_000 { (5_000,    1_000)   }
    else if len <     500_000 { (50_000,   10_000)  }
    else if len <   2_000_000 { (200_000,  50_000)  }
    else if len <  10_000_000 { (500_000,  100_000) }
    else                      { (1_000_000, 200_000) }
}
...
pub fn plasmid_to_svg(
    ...
    s.push_str(&format!("  <rect width=\"{0}\" height=\"{0}\" fill=\"white\"/>\n", size as u32));
    ...
    loop {
        let a = angle_of(pos, seq_len);
        let is_major = pos % major_step == 0;
        let (ri, ro) = if is_major { (r - 10.0, r + 10.0) } else { (r - 5.0, r + 5.0) };
    ...
    if show_features {
        for feat in &seq.features {
            ...
            let hex  = format!("#{:02X}{:02X}{:02X}",
                (fc.r * 255.0) as u8, (fc.g * 255.0) as u8, (fc.b * 255.0) as u8);
            ...
            if !feat.name.is_empty() {
                ...
                let label = if feat.name.len() > 16 { format!("{}…", &feat.name[..15]) } else { feat.name.clone() };
