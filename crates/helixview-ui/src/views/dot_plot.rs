...
use iced::{
    ...
    Alignment as IAlignment, Background, Color, Element, Font, Length, Pixels, Point, Rectangle,
    Size,
};
...
pub fn dot_plot_view<'a>(
    ...
    let win_slider = row![
        ...
        slider(1u32..=25, window as u32, |v| Message::DotPlotWindow(
            v as usize
        ))
        .width(80),
    ...
    let min_slider = row![
        ...
        slider(1u32..=min_max, cur_min, |v| Message::DotPlotMinMatch(
            v as usize
        ))
        .width(80),
    ...
    let dp_canvas = canvas(dp).width(Length::Fill).height(Length::Fill);
...
fn draw_dot_plot(
    ...
    let seq_a = match aln.sequences.get(a_idx) {
        Some(s) => s
            ...
            .collect::<Vec<u8>>(),
        None => {
            draw_placeholder(frame, size, "No sequence A");
            return;
        }
    };
    let seq_b = match aln.sequences.get(b_idx) {
        Some(s) => s
            ...
            .collect::<Vec<u8>>(),
        None => {
            draw_placeholder(frame, size, "No sequence B");
            return;
        }
    };
