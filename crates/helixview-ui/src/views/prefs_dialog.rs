...
use iced::{
    widget::{button, checkbox, column, container, row, scrollable, slider, text, horizontal_space},
...
pub fn prefs_dialog<'a>(prefs: &Preferences) -> Element<'a, Message> {
    ...
    let undo_row = row![
        ...
        slider(10..=500, prefs.undo_depth as u32, |v| Message::PrefsUndoDepth(v as usize))
            .width(Length::Fixed(180.0)),
        text(format!("{}", prefs.undo_depth)).size(12).color(palette::TEXT_DIM),
    ].spacing(8).align_y(Alignment::Center);

    // ── View defaults ─────────────────────────────────────────────────────────
    let zoom_row = row![
        text("Default zoom:").size(12).width(Length::Fixed(160.0)),
        slider(40..=300, (prefs.default_zoom * 100.0) as u32,
            |v| Message::PrefsDefaultZoom(v as f32 / 100.0))
            .width(Length::Fixed(180.0)),
        text(format!("{:.0}%", prefs.default_zoom * 100.0)).size(12).color(palette::TEXT_DIM),
    ].spacing(8).align_y(Alignment::Center);

    let font_row = row![
        text("Grid font size:").size(12).width(Length::Fixed(160.0)),
        slider(8..=20, prefs.font_size as u32, |v| Message::PrefsFontSize(v as f32))
            .width(Length::Fixed(180.0)),
        text(format!("{}pt", prefs.font_size as u32)).size(12).color(palette::TEXT_DIM),
    ].spacing(8).align_y(Alignment::Center);
    ...
    // ── Identity threshold ────────────────────────────────────────────────────
    let thresh_row = row![
        text("Identity threshold:").size(12).width(Length::Fixed(160.0)),
        slider(0.0..=1.0, prefs.identity_threshold,
    ...
    // ── Recent files ──────────────────────────────────────────────────────────
    let recent_row = row![
        text("Recent files to remember:").size(12).width(Length::Fixed(160.0)),
        slider(0..=20, prefs.recent_files_max as u32,
            |v| Message::PrefsRecentFilesMax(v as usize))
            .width(Length::Fixed(180.0)),
        text(format!("{}", prefs.recent_files_max)).size(12).color(palette::TEXT_DIM),
    ].spacing(8).align_y(Alignment::Center);
    ...
    if prefs.recent_files.is_empty() {
        recent_col = recent_col.push(
            text("No recent files.").size(11).color(palette::TEXT_DIM)
        );
    } else {
    ...
    let btn_row   = row![close_btn, horizontal_space(), save_btn]
