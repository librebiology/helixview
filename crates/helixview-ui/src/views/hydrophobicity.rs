...
pub fn hydrophobicity_view<'a>(aln: &'a Arc<SeqAlignment>, window: usize) -> Element<'a, Message> {
    ...
    let win_ctrl = row![
        ...
        slider(3u32..=25, window as u32, |v| Message::HydroWindow(
            v as usize
        ))
        .width(110),
    ...
    let toolbar = container(
        row![
            back_btn,
            text("Hydrophobicity (Kyte-Doolittle)")
                .size(14)
                .color(palette::TEXT),
            iced::widget::horizontal_space(),
            win_ctrl,
            text(format!("{n_seq} sequences"))
                .size(12)
...
fn draw_axes(frame: &mut Frame<iced::Renderer>, pw: f32, ph: f32, max_len: usize) {
    ...
    // Axis border.
    let border = canvas::Path::rectangle(Point::new(PAD_L, PAD_T), Size::new(pw, ph));
    frame.stroke(
        &border,
        canvas::Stroke::default()
...
fn draw_legend(frame: &mut Frame<iced::Renderer>, size: Size, aln: &SeqAlignment) {
    ...
    for (i, seq) in aln.sequences.iter().enumerate().take(12) {
        ...
        // Color swatch.
        frame.fill_rectangle(Point::new(x0, y), Size::new(14.0, 3.0), color);
        // Name.
