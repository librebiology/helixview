...
use crate::widgets::{AlignmentGrid, EntropyStrip, RestrMapStrip, PairingArcsStrip, PairingPair, STRIP_H, RESTR_H, ARCS_H};
use crate::widgets::grid::{CELL_W, CELL_H, FONT_SIZE};
...
pub fn alignment_view<'a>(
    ...
    let open_btn = button(text("Open").size(12)).padding([4, 12])
    ...
    let proj_open_btn = button(text("Open Project").size(12)).padding([3, 8])
        .style(btn_style::secondary)
        .on_press(Message::OpenProjectDialog);
    let proj_save_btn = button(text("Save Project").size(12)).padding([3, 8])
        .style(btn_style::secondary)
        .on_press(Message::SaveProjectDialog);
    let fetch_btn = button(text("Fetch…").size(12)).padding([3, 8])
        .style(btn_style::secondary)
        .on_press(Message::ToggleFetchBar);
    let export_btn = button(text("Export…").size(12)).padding([3, 8])
        .style(btn_style::secondary)
        .on_press(Message::ExportFastaDialog);
    let find_btn = button(text("Find  ⌕").size(12)).padding([3, 8])
        .style(btn_style::ghost)
        .on_press(Message::ToggleSearch);

    let zoom_out_btn = button(text("−").size(14)).padding([2, 7])
        .style(btn_style::ghost)
        .on_press(Message::ZoomOut);
    let zoom_in_btn  = button(text("+").size(14)).padding([2, 7])
        .style(btn_style::ghost)
        .on_press(Message::ZoomIn);
    let zoom_label = text(format!("{:.0}%", view.zoom * 100.0)).size(11).color(palette::TEXT_DIM);
    let zoom_row = row![zoom_out_btn, zoom_label, zoom_in_btn]
        .spacing(1).align_y(Alignment::Center);
    ...
    let tab_btn = |label: &str, tab: ToolbarTab| -> iced::widget::Button<'static, Message> {
        ...
        button(text(label.to_string()).size(12))
            .padding([4, 12])
            .style(if active { btn_style::tab_active } else { btn_style::tab_inactive })
    ...
    let toolbar_row1 = container(
        row![
            open_btn, save_btn,
            vsep(),
            proj_open_btn, proj_save_btn,
            vsep(),
            fetch_btn, export_btn,
            vsep(),
            doc_info,
            horizontal_space(),
            find_btn,
            ...
            zoom_row,
            vsep(),
            button(text("Prefs").size(12)).padding([3, 8])
                .style(btn_style::ghost)
                .on_press(Message::OpenPrefs),
            button(text("?").size(12)).padding([3, 8])
                .style(btn_style::ghost)
    ...
    let toolbar_row2 = {
        ...
        let inner: Element<Message> = match view.toolbar_tab {
            ToolbarTab::Edit => {
                let features_label = if view.show_features { "Features ●" } else { "Features ○" };
                let restr_label    = if show_restr_map     { "RE Map ●"   } else { "RE Map ○"   };
                let feat_style = if view.show_features { btn_style::toggle_on } else { btn_style::toggle_off };
                let re_style   = if show_restr_map     { btn_style::toggle_on } else { btn_style::toggle_off };
                row![
                    ...
                    button(text(features_label).size(12)).padding([3, 9]).style(feat_style).on_press(Message::ToggleFeatures),
                    button(text(restr_label).size(12)).padding([3, 9]).style(re_style).on_press(Message::ToggleRestrMap),
                ]
                .spacing(4)
                .align_y(Alignment::Center)
                .into()
            ...
            ToolbarTab::Analysis => {
                let protein_label = if view.protein_view_frame.is_some() { "< DNA" } else { "Protein" };
            ...
            ToolbarTab::View => {
                let analysis_label = if view.show_analysis { "Side <" } else { "Side >" };
                let arc_style = if show_pairing_arcs { btn_style::toggle_on } else { btn_style::toggle_off };
                let arc_label = if show_pairing_arcs { "Cov Arcs ●" } else { "Cov Arcs ○" };
                row![
                    ...
                    vsep(),
                    button(text(arc_label).size(12)).padding([3, 9]).style(arc_style)
                        .on_press(Message::TogglePairingArcs),
                    iced::widget::slider(
                        1u32..=20,
                        pairing_threshold as u32,
                        |v| Message::PairingArcThreshold(v as f64),
                    ).width(70),
                    text(format!("≥{:.0}", pairing_threshold)).size(10).color(palette::TEXT_DIM),
                    vsep(),
                    sec("Accessories",    Message::ShowAccessories),
                    sec("Create BLAST DB", Message::CreateBlastDbDialog),
                    vsep(),
                    sec("ClustalW",  Message::RunExternalAlign(ExternalAligner::ClustalW)),
                    sec("ClustalΩ",  Message::RunExternalAlign(ExternalAligner::ClustalOmega)),
    ...
    let grid_canvas = canvas(grid)
        .width(Length::Fill)
        .height(Length::Fill);

    // ── Covariation pairing arcs strip (optional) ────────────────────────────
    // Four states: off | no MI (run hint) | stale MI (re-run hint) | show arcs.
    let pairing_strip: Option<Element<Message>> =
        if show_pairing_arcs && !mi_available {
            drop(pairing_pairs);
            Some(
                container(
                ...
                .style(|_| container::Style {
                    ...
                    border: iced::Border { color: Color::from_rgb(0.80, 0.78, 0.90), width: 1.0, radius: 0.0.into() },
                    ..Default::default()
                })
            ...
            Some(
                container(
                    text("⚠ MI results are stale — alignment changed since last run. Re-run to update.")
                        .size(10)
                ...
                .style(|_| container::Style {
                    ...
                    border: iced::Border { color: Color::from_rgb(0.80, 0.65, 0.30), width: 1.0, radius: 0.0.into() },
                    ..Default::default()
                })
                .align_y(iced::alignment::Vertical::Center)
                .padding([0, 8])
    ...
    let approx_vis = ((100.0 / view.zoom).round() as usize).max(10).min(total_cols);
    ...
    let h_scroll: Element<Message> = if max_col > 0 {
        ...
        // ── Scroll buttons ──
        let btn_start = button(text("|<").size(11)).padding([3, 7]).style(bs::ghost)
            .on_press(Message::ScrollColSet(0));
        let btn_back  = button(text("<").size(12)).padding([3, 7]).style(bs::ghost)
            .on_press(Message::ScrollColSet(cur_col.saturating_sub(20)));
        let btn_fwd   = button(text(">").size(12)).padding([3, 7]).style(bs::ghost)
            .on_press(Message::ScrollColSet((cur_col + 20).min(scroll_max)));
        let btn_end   = button(text(">|").size(11)).padding([3, 7]).style(bs::ghost)
        ...
        .style(|_| container::Style {
            background: Some(Background::Color(palette::BORDER)),
            border: Border { radius: 3.0.into(), color: palette::BORDER_STRONG, width: 1.0 },
            ..Default::default()
        });
    ...
    let hover_info = match (view.hover_col, view.hover_row) {
        (Some(c), Some(r)) => {
            let residue_ch = aln.sequences.get(r)
            ...
            let freq_info = if aln.seq_count() <= 500 {
                ...
                if total > 0 {
                    let mut sorted: Vec<(u8, u32)> = counts.iter()
                        .enumerate()
                        .filter(|(_, &n)| n > 0)
                        .map(|(i, &n)| (i as u8, n))
                        .collect();
                    sorted.sort_unstable_by(|a, b| b.1.cmp(&a.1));
                    let top: String = sorted.iter().take(3)
                        .map(|(b, n)| format!("{}:{:.0}%", *b as char, *n as f32 / total as f32 * 100.0))
                        .collect::<Vec<_>>().join(" ");
                    format!("  [{}]", top)
                } else {
                    String::new()
            ...
            format!("  ·  col {} row {}{}{}", c + 1, r + 1, residue_ch, freq_info)
        }
        _ => String::new(),
    };
    ...
    let protein_banner: Option<Element<Message>> = if let Some(frame) = view.protein_view_frame {
        ...
        let frame_btns = row![
            button(text("F+1").size(10)).padding([1,5]).on_press(Message::SetProteinViewFrame(0)),
            button(text("F+2").size(10)).padding([1,5]).on_press(Message::SetProteinViewFrame(1)),
            button(text("F+3").size(10)).padding([1,5]).on_press(Message::SetProteinViewFrame(2)),
            button(text("F-1").size(10)).padding([1,5]).on_press(Message::SetProteinViewFrame(3)),
            button(text("F-2").size(10)).padding([1,5]).on_press(Message::SetProteinViewFrame(4)),
            button(text("F-3").size(10)).padding([1,5]).on_press(Message::SetProteinViewFrame(5)),
        ].spacing(2);
        Some(
            container(
                row![
            ...
            .style(|_| container::Style {
                ...
                border: Border { color: Color::from_rgb(0.90, 0.70, 0.20), width: 1.0, radius: 0.0.into() },
                ..Default::default()
            })
            .width(Length::Fill)
            .into(),
        ...
        Some(
            container(
                row![
                    ...
                    button(text(if view.blast_program == "blastn" { "blastn ●" } else { "blastn" }).size(11))
                        .padding([2,6]).on_press(Message::BlastProgramChanged("blastn".into())),
                    button(text(if view.blast_program == "blastp" { "blastp ●" } else { "blastp" }).size(11))
                        .padding([2,6]).on_press(Message::BlastProgramChanged("blastp".into())),
                    button(text(if view.blast_program == "blastx" { "blastx ●" } else { "blastx" }).size(11))
                        .padding([2,6]).on_press(Message::BlastProgramChanged("blastx".into())),
                    text("DB:").size(11).color(palette::TEXT_DIM),
                    button(text(if view.blast_database == "nt" { "nt ●" } else { "nt" }).size(11))
                        .padding([2,6]).on_press(Message::BlastDatabaseChanged("nt".into())),
                    button(text(if view.blast_database == "nr" { "nr ●" } else { "nr" }).size(11))
                        .padding([2,6]).on_press(Message::BlastDatabaseChanged("nr".into())),
                    button(text(if view.blast_database == "refseq_rna" { "refseq_rna ●" } else { "refseq_rna" }).size(11))
                        .padding([2,6]).on_press(Message::BlastDatabaseChanged("refseq_rna".into())),
                    iced::widget::horizontal_space(),
            ...
            .style(|_| container::Style {
                ...
                border: Border { color: Color::from_rgb(0.70, 0.40, 0.90), width: 1.0, radius: 0.0.into() },
                ..Default::default()
            })
            .width(Length::Fill)
            .into(),
        ...
        Some(
            container(
                row![
                    ...
                    text_input("Accession — NCBI (NM_001256799) or UniProt (P02945), comma-separated…", &view.fetch_input)
    ...
    let grid_col: Element<Message> = {
        ...
        if let Some(pb) = protein_banner { parts.push(pb); }
        if let Some(bp) = blast_panel    { parts.push(bp); }
        if let Some(sb) = search_bar     { parts.push(sb); }
        if let Some(fb) = fetch_bar      { parts.push(fb); }
        if let Some(ab) = annotate_bar   { parts.push(ab); }
        parts.push(grid_canvas.into());
        if let Some(rs) = restr_strip   { parts.push(rs.into()); }
        if let Some(ps) = pairing_strip { parts.push(ps.into()); }
        parts.push(entropy_strip.into());
        column(parts).width(Length::Fill).height(Length::Fill).into()
    };
    ...
    let ctx_menu: Option<Element<Message>> = title_menu.and_then(|idx| {
        ...
        let panel = container(
            column![
                row![
                    text(format!("Sequence: {}", seq.name)).size(12).color(palette::TEXT),
                    horizontal_space(),
                    button(text("✕").size(11)).padding([2, 6]).on_press(Message::TitleMenuClose),
                ]
                .spacing(6)
                .align_y(Alignment::Center),
                row![
                    ...
                    text_input("new name…", rename_input)
                        ...
                        .on_submit(Message::RenameSequence { idx, name: name_for_rename })
                        .size(11)
                        .width(Length::Fill),
                    button(text("OK").size(11))
                        .padding([2, 8])
                        .on_press(Message::RenameSequence { idx, name: rename_input.to_owned() }),
                ]
                .spacing(6)
                .align_y(Alignment::Center),
                row![
                    button(text(lock_label).size(11)).padding([3, 8])
                        .on_press(Message::ToggleLockSequence { idx }),
                    button(text("Duplicate").size(11)).padding([3, 8])
                        .on_press(Message::DuplicateSequence { idx }),
                    button(text("Delete").size(11)).padding([3, 8])
                        .on_press(Message::DeleteSequence { idx }),
                    button(text("Run ORFs").size(11)).padding([3, 8])
                        .on_press(Message::RunOrfFinder),
                    button(text("Rev-Comp").size(11)).padding([3, 8])
    ...
    if let Some(menu) = ctx_menu { layout_parts.push(menu); }
...
fn scheme_btn<'a>(
    ...
    button(text(label).size(12))
        .padding([3, 9])
        .on_press(msg)
        .style(if active { bs::toggle_on } else { bs::toggle_off })
        .into()
}
