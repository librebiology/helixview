...
pub async fn submit(
    ...
    let query_str = std::str::from_utf8(query)
        .map_err(|_| "Query contains non-UTF-8 bytes".to_string())?;
...
fn parse_tabular(text: &str) -> Result<Vec<BlastHit>, String> {
    ...
    for line in text.lines() {
        ...
        if hits.iter().any(|h: &BlastHit| h.accession == accession) { continue; }
...
fn make_client() -> Result<reqwest::Client, String> {
    reqwest::Client::builder()
        .user_agent(concat!("HelixView/", env!("CARGO_PKG_VERSION"), " (NCBI BLAST)"))
