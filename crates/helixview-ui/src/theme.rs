...
pub mod buttons {
    use iced::{Background, Border, Color, Shadow, Vector, widget::button};
    use super::palette;

    fn shadow_sm() -> Shadow {
        Shadow { color: Color::from_rgba(0.0, 0.0, 0.0, 0.12), offset: Vector::new(0.0, 1.5), blur_radius: 3.0 }
    }
    ...
    pub fn secondary(_t: &iced::Theme, s: button::Status) -> button::Style {
        let (bg, text) = match s {
            button::Status::Hovered  => (Color::from_rgb(0.83, 0.85, 0.93), Color::from_rgb(0.08, 0.08, 0.18)),
            button::Status::Pressed  => (Color::from_rgb(0.74, 0.76, 0.86), Color::from_rgb(0.05, 0.05, 0.14)),
            button::Status::Disabled => (Color::from_rgba(0.88, 0.90, 0.95, 0.5), Color::from_rgba(0.30, 0.30, 0.38, 0.5)),
            _                        => (Color::from_rgb(0.89, 0.91, 0.96), Color::from_rgb(0.16, 0.16, 0.26)),
        };
        button::Style {
            background: Some(Background::Color(bg)),
            text_color: text,
    ...
    pub fn ghost(_t: &iced::Theme, s: button::Status) -> button::Style {
        let (bg, text) = match s {
            button::Status::Hovered  => (Color::from_rgba(0.0, 0.0, 0.0, 0.07), Color::from_rgb(0.10, 0.10, 0.20)),
            button::Status::Pressed  => (Color::from_rgba(0.0, 0.0, 0.0, 0.12), Color::from_rgb(0.05, 0.05, 0.14)),
    ...
    pub fn tab_inactive(_t: &iced::Theme, s: button::Status) -> button::Style {
        let (bg, text) = match s {
            button::Status::Hovered  => (Color::from_rgb(0.82, 0.84, 0.92), Color::from_rgb(0.08, 0.08, 0.22)),
            button::Status::Pressed  => (Color::from_rgb(0.76, 0.78, 0.88), Color::from_rgb(0.05, 0.05, 0.18)),
            _                        => (Color::from_rgb(0.90, 0.91, 0.96), Color::from_rgb(0.26, 0.28, 0.40)),
        };
        button::Style {
            background: Some(Background::Color(bg)),
            text_color: text,
